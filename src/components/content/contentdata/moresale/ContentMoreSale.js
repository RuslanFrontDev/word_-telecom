...
import MoreFirstProduct from "./morefirstProduct/MoreFirstProduct";
import { addToLikeCart } from "../../../../redux/stores/features/likeSlice";
import { addToNowBuyCart } from "../../../../redux/stores/features/buySlice";
const ContentMoreSale = () => {
  ...
  //likeCart
  const handleAddToLikeCart = (moreproduct) => {
    dispatch(addToLikeCart(moreproduct));
    navigation("/likecart");
  };
  const handleNowBuy = (moreproduct) => {
    dispatch(addToNowBuyCart(moreproduct))
    navigation("/nowbuy", {
      replace: true,
    });
  };
  ...
  //  },[])
  //  const search = useSelector((state) => state.search.category);
  //  const filteredData = moreproductsData.moreproducts.filter((product) => {
  //    return product.productName.toLowerCase().includes(search.toLowerCase());
  //  });
  //  console.log(filteredData);
  //slice
  const arr = filteredDatas?.slice(0, 8);
    ...
    <>
      ...
      <div className="contentMoreSale">
        <div className="textMoreSale">
          <span className="textMoreSale-title">Ən çox satılanlar</span>
        </div>
                  ...
                  <div className="cart-cartProduct">
                    <div className="cart-cartProductPadding">
                      ...
                      <div className="cart-like">
                        <button
                          onClick={() => handleAddToLikeCart(moreproduct)}
                      ...
                      <div className="product-buyNowContainer">
                        <button className="NowBuyPro" onClick={() => handleNowBuy(moreproduct)}>
                          indi al
                        </button>
                        <div className="cart-cartIconSbt">
                          <button
                            className="basketGive"
                            onClick={() => handleAddToCart(moreproduct)}
                          >
                            <h4>Sebete at</h4>
...
export default ContentMoreSale;
